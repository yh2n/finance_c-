...
void printAmortHeader() {
    cout <<
        endl <<
        setw(kYearOut) <<AmortHeader [0]<< " " <<
        setw(kMinOut) << AmortHeader [1] << " " <<
        setw(kCumMinOut) << AmortHeader [2] << " " <<
        setw(kPaymentOut) << AmortHeader [3] << " " <<
        setw(kInterestOut) << AmortHeader [4] << " " <<
        setw(kPrincipalOut) << AmortHeader [5] << " " <<
        setw(kBalanceOut) << AmortHeader [6] << " " << 
        endl <<
        endl;
};
...
int main(int argc, char *argv[]) {
   ...
   while(curr_loan_month <= months_of_loan) {
    ...
    // 'abs' to get rid of negative sign
    // curr_balance = abs(curr_balance - curr_principal_payment);
